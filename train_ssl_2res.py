...
from datasets.ucf101 import UCF101Dataset_2clips as UCF101Dataset
from datasets.hmdb51 import HMDB51Dataset
from models.c3d import C3D
from models.r21d import R2Plus1DNet
from models.r3d import R3DNet
from models.s3d import S3D
from models.network import IICnet
...
def parse_option():
    ...
    # model definition
    parser.add_argument('--model', type=str, default='r3d', choices=['r3d', 'c3d', 'r21d', 'r18', 's3d'])
    ...
    parser.add_argument('--feat_dim', type=int, default=512, help='dim of network output')
    parser.add_argument('--proj_dim', type=int, default=128, help='dim of feat for inner product')
    parser.add_argument('--head', type=ast.literal_eval, default=True)
    ...
    parser.add_argument('--modality', type=str, default='res', choices=['rgb', 'res', 'rgb+res'])
    parser.add_argument('--intra_neg', type=ast.literal_eval, default=True)
    parser.add_argument('--neg', type=str, default='shuffle', choices=['repeat', 'shuffle', 'rot', 'rot90'])
    parser.add_argument('--desp', type=str)
    parser.add_argument('--seed', type=int, default=632)
    parser.add_argument('--aug', type=ast.literal_eval, default=True)

    # schedule
    parser.add_argument('--schedule', default=[120, 160], nargs='*', type=int,
                    help='learning rate schedule (when to drop lr by 10x)')
    parser.add_argument('--cos', type=ast.literal_eval, default=True, help='use cosine lr schedule')

    opt = parser.parse_args()

    if opt.intra_neg:
        print('[Warning] using intra-negative')
        opt.model_name = 'Aug_IIC_{}_{}_{}_lr{}_k{}_{}'.format(opt.model, opt.neg, opt.modality, opt.lr, opt.nce_k, time.strftime('%m%d'))
    else:
        print('[Warning] using baseline')
        opt.model_name = 'Aug_{}_{}_lr{}_k{}_{}'.format(opt.model, opt.modality, opt.lr, opt.nce_k, time.strftime('%m%d'))
    
    if opt.desp:
        opt.model_name = opt.model_name + opt.desp

    if not opt.aug:
        opt.model_name = 'No' + opt.model_name
...
def set_model(args, n_data):
    # set the model
    if args.model == 'c3d':
        base_network = C3D(with_classifier=False)
    elif args.model == 'r3d':
        base_network = R3DNet(layer_sizes=(1,1,1,1), with_classifier=False)
    elif args.model == 'r21d':  
        base_network = R2Plus1DNet(layer_sizes=(1,1,1,1), with_classifier=False)
    elif args.model == 'r18':
        torchmodel = torchvision.models.video.r3d_18(pretrained=False, progress=True)
        base_network = torch.nn.Sequential(*(list(torchmodel.children())[:-1]), View())
    elif args.model == 's3d':
        base_network = S3D(with_classifier=False)
        args.feat_dim = 1024
    
    model = IICnet(base_network, feature_size=args.feat_dim, head=args.head, proj_dim=args.proj_dim)

    if args.intra_neg:
        contrast = NCEAverage(args.proj_dim, n_data, args.nce_k, args.nce_t, args.nce_m, args.softmax)
    else:
        contrast = NCEAverage_ori(args.proj_dim, n_data, args.nce_k, args.nce_t, args.nce_m, args.softmax)
...
def train(epoch, train_loader, model, contrast, criterion_1, criterion_2, optimizer, opt):
    ...
    for idx, (inputs1, inputs2, _, index) in enumerate(train_loader):
        ...
        # ===================forward=====================
        if opt.modality == 'res':
            feat_1 = model(diff(inputs1))
            feat_2 = model(diff(inputs2))
        elif opt.modality == 'rgb':
            feat_1 = model(inputs1)
        ...
        if not opt.intra_neg:
            ...
            feat_neg = model(preprocess(diff(inputs1), opt.neg))
...
def main():
    ...
    # use func adjust_learning_rate instead, follow MoCo
