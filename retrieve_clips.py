...
def extract_feature(args):
    ...
    ########### model ##############
    if args.model == 'c3d':
        model = C3D(with_classifier=False, return_conv=args.convfeat).to(device)
    elif args.model == 'r3d':
        model = R3DNet(layer_sizes=(1,1,1,1), with_classifier=False, return_conv=args.convfeat).to(device)
    elif args.model == 'r21d':
        model = R2Plus1DNet(layer_sizes=(1,1,1,1), with_classifier=False, return_conv=args.convfeat).to(device)
    elif args.model == 's3d':
        model = S3D(with_classifier=False).to(device)
        args.featdim = 1024
    elif args.model == 'r18':
        torchmodel = torchvision.models.video.r3d_18(pretrained=False, progress=True)
        model = torch.nn.Sequential(*(list(torchmodel.children())[:-1]), View()).to(device)
        args.featdim = 512

    #model = nn.DataParallel(model)
    if args.ckpt:
        pretrained_weights = adjust_weights(torch.load(args.ckpt)[args.key])
...
def topk_retrieval(args):
    ...
    f_record3 = open(os.path.join(args.feature_dir, 'top3_results.txt'), 'w')
    top_3_indices = indices[:, :3]
    # ind should start from 0
    f_record3.write('query\t1\t2\t3\tindex\ti1\ti2\ti3\n')
    cnt = 0
    for ind, test_label in zip(top_3_indices, y_test):
        labels = y_train[ind]
        indexs = ind
        s = "{0}\t{1}\t{2}\t{3}\t\t{4}\t{5}\t{6}\t{7}\n".format(test_label,labels[0],labels[1],labels[2],cnt,indexs[0],indexs[1],indexs[2])
        f_record3.write(s)
        cnt = cnt + 1
    f_record3.close()
...
def parse_args():
    ...
    parser.add_argument('--key', type=str, default='model', help='model key')
    ...
    parser.add_argument('--convfeat', default=True, type=ast.literal_eval, help='If False, return 512 feature.') 
