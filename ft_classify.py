...
def adjust_weights_module(weights):
    """load pretrained weights and adjust params name."""
    adjusted_weights = {}
    for name, params in weights.items():
        if 'module' in name:
...
def test(args, model, criterion, test_dataloader):
    ...
    return avg_loss, accuracies.avg
...
def parse_args():
    ...
    parser.add_argument('--optim', default='SGD', type=str, help='try adam') 
...
if __name__ == '__main__':
    ...
    if args.dataset == 'ucf101':
        ...
        class_num = 51
    elif args.dataset == 'kinetics' or args.dataset == 'k400':
        class_num = 400
    ...
    if args.model == 'c3d':
        ...
        model = R18(with_classifier=True, num_classes=class_num).cuda()
    elif args.model == 's3d':
        model = S3D(with_classifier=True, num_classes=class_num).cuda()
    #pretrained_weights = load_pretrained_weights(args.ckpt)
    pretrained_weights = torch.load(args.ckpt)
    if args.mode == 'train':
        if args.model == 'r18':
            model.load_state_dict(adjust_weights_module(pretrained_weights['model']), strict=False)
        else:
            model.load_state_dict(adjust_weights(pretrained_weights['model']), strict=False)
    else:
    ...
    if args.desp:
        exp_name = '{}_{}_cls_{}_split{}_{}_{}'.format(args.model, args.modality, args.dataset, args.split, args.desp, time.strftime('%m%d'))
    else:
        exp_name = '{}_{}_cls_{}_split{}_{}'.format(args.model, args.modality, args.dataset, args.split, time.strftime('%m%d'))
    print(exp_name)
    ...
    if args.dataset == 'ucf101':
        ...
        val_size = 400
    elif args.dataset == 'kinetics400' or args.dataset == 'k400':
        train_dataset = Kinetics400Dataset('data/k400', args.cl, args.split, True, train_transforms)
        test_dataset = Kinetics400Dataset('data/k400', args.cl, args.split, False, test_transforms)
    ...
    if args.optim == 'adam':
        print('Warning: use adam to optimize network')
    ...
    _, test_acc = test(args, model, criterion, test_dataloader)

    with open('results.txt','a+') as f:
        f.write('{}\t{}\t{}\t{}\n'.format(exp_name, args.dataset, args.split, test_acc))
