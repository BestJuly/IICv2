...
class UCF101Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #framefolder = os.path.join('/raid/home/taoli/dataset/ucf101/jpegs_256', vid)
        framefolder = os.path.join('/raid/dataset/ucf101/jpegs_256', vid)
...
class UCF101VCOPDataset(Dataset):
    ...
    def __init__(self, root_dir, clip_len, interval, tuple_len, train=True, transforms_=None):
        ...
        if self.train:
            vcop_train_split_name = 'vcop_train_{}_{}_{}.txt'.format(clip_len, interval, tuple_len)
            vcop_train_split_path = os.path.join(root_dir, vcop_train_split_name)
            self.train_split = pd.read_csv(vcop_train_split_path, header=None)[0]
        else:
            vcop_test_split_name = 'vcop_test_{}_{}_{}.txt'.format(clip_len, interval, tuple_len)
            vcop_test_split_path = os.path.join(root_dir, vcop_test_split_name)
            self.test_split = pd.read_csv(vcop_test_split_path, header=None)[0]
    ...
    def __getitem__(self, idx):
        ...
        length = len(framenames)
        if length < 64:
            print(framefolder, length, self.tuple_total_frames)
        

        tuple_order = list(range(0, self.tuple_len)) # default tuple_len=3
        ...
        clip_start = tuple_start
        tuple_clip = load_from_frames(framefolder, framenames, tuple_start, self.tuple_len, self.clip_len, self.interval)
        ...
        return torch.stack(tuple_clip), torch.tensor(tuple_order), torch.tensor(int(idx))
...
class UCF101VCPDataset_new(Dataset):
    """UCF101 dataset for video clip order prediction. Generate clips and permutes them on-the-fly.
    Need the corresponding configuration file exists. 
...
def load_one_clip_angle(foldername, framenames, start_index, clip_len):
    one_clip = []
    for i in range(clip_len):
        im_name = os.path.join(foldername, framenames[start_index + i])
        im_data = readim(im_name)
        one_clip.append(im_data)
    one_clip = np.array(one_clip) # 16 x H x W x C
    angle = np.random.randint(4)
    clip_rot = np.rot90(one_clip, k=angle, axes=(1,2))
    return clip_rot, angle


class UCF101Dataset_2Clips(Dataset):
    """UCF101 dataset for recognition. The class index start from 0.
    ...
    def __getitem__(self, idx):
        ...
        if self.train or self.val:
            clip_start = random.randint(0, length - self.clip_len)
            clip1, angle1 = load_one_clip_angle(framefolder, framenames, clip_start, self.clip_len)
            clip_start = random.randint(0, length - self.clip_len)
            clip2, angle2 = load_one_clip_angle(framefolder, framenames, clip_start, self.clip_len)
            #clip = videodata[clip_start: clip_start + self.clip_len]
            ...
            if self.transforms_:
                ...
                for frame in clip1:
                    ...
                    frame = self.transforms_(frame) # tensor [C x H x W]
                    trans_clip.append(frame)
                clip1 = torch.stack(trans_clip).permute([1, 0, 2, 3]) # (T x C X H x W) to (C X T x H x W)

                # clip 2, repeat codes for clip 1
                trans_clip = []
                seed = random.random()
                for frame in clip2:
                    random.seed(seed)
                    ...
                    frame = self.transforms_(frame) # tensor [C x H x W]
                    trans_clip.append(frame)
                clip2 = torch.stack(trans_clip).permute([1, 0, 2, 3]) # (T x C X H x W) to (C X T x H x W)
            else:
                clip1 = torch.tensor(clip1)
                clip2 = torch.tensor(clip2)
            angle1 = torch.tensor(angle1)
            angle2 = torch.tensor(angle2)

            return clip1, angle1, clip2, angle2, idx, torch.tensor(int(class_idx))
        # sample several clips for test
        else:
            all_clips = []
            all_idx = []
            ...
            for i in np.linspace(self.clip_len/2, length-self.clip_len/2, self.test_sample_num):
                clip_start = int(i - self.clip_len/2)
                clip, angle = load_one_clip_angle(framefolder, framenames, clip_start, self.clip_len)
                if self.transforms_:
                    trans_clip = []
                    # fix seed, apply the sample `random transformation` for all frames in the clip 
                    seed = random.random()
            ...
            return torch.stack(all_clips), torch.stack(angles), torch.tensor(int(class_idx))



# Kinetics400 dataset
## VCP
class Kinetics400_VCPDataset(Dataset):
    """Kinetics dataset for video clip order prediction. Generate clips and permutes them on-the-fly.
    Need the corresponding configuration file exists. 
    ...
    def __init__(self, root_dir, clip_len, interval, tuple_len, train=True, transforms_=None):
        ...
        if self.train:
            train_split_path = os.path.join(root_dir, 'k400/vcp_train.list')
            self.train_split = list(open(train_split_path, 'r'))
        else:
            test_split_path = os.path.join(root_dir, 'k400/vcp_val.list')
            self.test_split = list(open(test_split_path, 'r'))
    ...
    def __getitem__(self, idx):
        ...
        # videoname: ./dataset/kinetics/train/air_drumming/sJFLvijNdsg_000000_000010 1
        videoname = videoname.strip()
        vid = videoname.split(' ')[0][19:]
        class_idx = int(videoname.split(' ')[-1])

        framefolder = os.path.join('/raid/dataset/kinetics400/', vid)
        ...
        framenames = filenames
        length = len(framenames)
        #print(vid, length, self.tuple_total_frames)
...
## 3DRotNet
class Kinetics400_2Clips(Dataset):
    """UCF101 dataset for recognition. The class index start from 0.
    ...
    def __init__(self, root_dir, clip_len=16, split='1', train=True, val=False, transforms_=None, test_sample_num=10):
        ...
        if self.train:
            train_split_path = os.path.join(root_dir, 'k400/train.list')
            self.train_split = list(open(train_split_path, 'r'))
        else:
            test_split_path = os.path.join(root_dir, 'k400/val.list')
            self.test_split = list(open(test_split_path, 'r'))
    ...
    def __getitem__(self, idx):
        ...
        videoname = videoname.strip()
        vid = videoname.split(' ')[0][19:]
        class_idx = 0 # do not use labels, this will be deleted in a clearer code version
