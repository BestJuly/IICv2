...
class UCF101Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #u_folder = os.path.join('/work/taoli/ucf101/jpegs_256/', vid)
        #v_folder = os.path.join('/work/taoli/ucf101/jpegs_256/', vid)
        #u_folder = os.path.join('/work/taoli/ucf101/tvl1_flow/u/', vid)
        #v_folder = os.path.join('/work/taoli/ucf101/tvl1_flow/v/', vid)
        
        jpegs_path = '/raid/dataset/ucf101/jpegs_256'
        rgb_folder = os.path.join(jpegs_path, vid) # + v_**
        u_folder = os.path.join(jpegs_path, vid) # + v_**
        v_folder = os.path.join(jpegs_path, vid) # + v_**
...
class UCF101ClipRetrievalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #u_folder = os.path.join('/work/taoli/ucf101/jpegs_256/', vid) # + v_**
        #v_folder = os.path.join('/work/taoli/ucf101/jpegs_256/', vid) # + v_**
        #u_folder = os.path.join('/work/taoli/ucf101/tvl1_flow/u/', vid)
        #v_folder = os.path.join('/work/taoli/ucf101/tvl1_flow/v/', vid)

        jpegs_path = '/raid/home/taoli/dataset/ucf101/jpegs_256'
        rgb_folder = os.path.join(jpegs_path, vid) # + v_**
        u_folder = os.path.join(jpegs_path, vid) # + v_**
        v_folder = os.path.join(jpegs_path, vid) # + v_**
...
class UCF101Dataset_2clips(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #rgb_folder = os.path.join('/work/taoli/ucf101/jpegs_256/', vid)
        rgb_folder = os.path.join('/raid/home/taoli/dataset/ucf101/jpegs_256/', vid)
        ...
        # random select a clip for train
        if self.train:
            clip_start1 = random.randint(0, length - self.clip_len)
            clip1 = load_one_clip(rgb_folder, framenames, clip_start1, self.clip_len)
            clip_start2 = random.randint(0, length - self.clip_len)
            clip2 = load_one_clip(rgb_folder, framenames, clip_start2, self.clip_len)
            ...
            if self.transforms_:
                ...
                for i in range(len(clip1)):
                    random.seed(seed)
                    frame1 = self.toPIL(clip1[i]) # PIL image
                    frame1 = self.transforms_(frame1) # tensor [C x H x W]
                    trans_clip1.append(frame1)

                    frame2 = self.toPIL(clip2[i]) # PIL image
                    frame2 = self.transforms_(frame2) # tensor [C x H x W]
                    trans_clip2.append(frame2)

                # (T x C X H x W) to (C X T x H x W)
                clip1 = torch.stack(trans_clip1).permute([1, 0, 2, 3])
                clip2 = torch.stack(trans_clip2).permute([1, 0, 2, 3])
            else:
                clip1 = torch.tensor(clip1)
                clip2 = torch.tensor(clip2)

            return clip1, clip2, torch.tensor(int(class_idx)), idx
        # sample several clips for test
        else:
            all_clips = []
            all_idx = []
            ...
            return torch.stack(all_clips), torch.tensor(int(class_idx)), idx


# for Kinetics-400 dataset
class Kinetics400Dataset_2clips(Dataset):
    """UCF101 dataset for recognition. The class index start from 0.
    Args:
        root_dir (string): Directory with videos and splits.
        train (bool): train split or test split.
