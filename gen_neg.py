...
def rotation(x, type='random'): 
    c, t, h, w = x.shape
    if type == 'random':
        x_new = torch.rot90(x, random.randint(0,2) + 1, [2, 3])
    else:
        x_new = torch.rot90(x, 1, [2, 3])
    return x_new
...
def preprocess(inputs, option='repeat'):
    ...
    for i in range(b):
        ...
        if option == 'repeat':
            ...
            new_in.append(rotation(one_sample, type='random'))
        elif option == 'rot90':
            new_in.append(rotation(one_sample, type='90'))
